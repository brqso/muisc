...
async def playlist(client, message):
    ...
    msg = "**مشغل الان** in {}".format(message.chat.title)
    msg += "\n- " + now_playing
    msg += "\n- مطلوب من قبل " + by
    temp.pop(0)
    if temp:
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("❌ إغلاق", "cls")],
        ]
    )
    return mar
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "<b>مشغل الان</b> in {}".format(cb.message.chat.title)
        msg += "\n- " + now_playing
        msg += "\n- مطلوب من قبل " + by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return
    lel = await message.reply("🔄 <b>جاري المعالجة</b>")
...
async def ytplay(_, message: Message):
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>مشغل</b> here the song rطلب من {} via Youtube Music 😎".format(
                message.from_user.mention()
            ),
        )
        os.remove("final.png")
...
async def lol_cb(b, cb):
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await b.send_photo(
            ...
            photo="final.png",
            reply_markup=keyboard,
            caption=f"▶️ <b>مشغل</b> here the song طلب من {r_by.mention()} via Youtube Music 😎",
        )
        os.remove("final.png")
