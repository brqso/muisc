...
async def playlist(client, message):
    ...
    msg = "<b>مشغل الان</b> in {}".format(lel.linked_chat.title)
    msg += "\n- " + now_playing
    msg += "\n- طلب من " + by
    temp.pop(0)
    if temp:
...
async def updated_stats(chat, queue, vol=100):
    if chat.id in pytgcalls.active_chats:
        ...
        if len(que) > 0:
            stats += "\n\n"
            stats += "الصوت : {}%\n".format(vol)
            stats += "الأغاني في قائمة الانتظار : `{}`\n".format(len(que))
            stats += "مشغل الان : **{}**\n".format(queue[0][0])
    ...
    mar = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("⏸", "ايقاف مؤقت"),
                InlineKeyboardButton("▶️", "استمرار"),
                InlineKeyboardButton("⏭", "تخطي"),
            ],
            [
                InlineKeyboardButton("قائمة التشغيل 📖", "cplaylist"),
            ],
            [InlineKeyboardButton("❌ إغلاق", "ccls")],
        ]
    )
    return mar
    ...
    if type_ == "cpause":
        ...
        marr = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("⏹", "توقف"),
                    InlineKeyboardButton("⏸", "إيقاف مؤقت"),
                    InlineKeyboardButton("▶️", "استمرار"),
                    InlineKeyboardButton("⏭", "تخطي"),
                ],
                [
...
async def play(_, message: Message):
    ...
    try:
        ...
        return
    text_links = None
    await lel.edit("🔎 <b>جاري المعالجة</b>")
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("القائمة ⏯ ", callback_data="cmenu"),
                ],
                [InlineKeyboardButton(text="❌ إغلاق", callback_data="ccls")],
            ]
        )
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [
                    ...
                    InlineKeyboardButton(text="جاري التحميل 📥", url=f"{dlurl}"),
                ],
                [InlineKeyboardButton(text="❌ إغلاق", callback_data="ccls")],
            ]
        )
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>مشغل</b> the song طلب من {} via Youtube Music 😎 in Linked Channel".format(
