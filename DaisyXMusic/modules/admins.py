...
async def pause(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("❗ لا شي مشغل!")
    else:
        await pytgcalls.pause_stream(chat_id)
        await message.reply_text("▶️ استمرار!")
...
async def resume(_, message: Message):
    ...
    for x in pytgcalls.active_calls:
        ACTV_CALLS.append(int(x.chat_id))
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("❗ لا شي متوقف مؤقتاً!")
    else:
        await pytgcalls.resume_stream(chat_id)
        await message.reply_text("⏸ ايقاف مؤقت!")
...
async def skip(_, message: Message):
    ...
    for x in pytgcalls.active_calls:
        ACTV_CALLS.append(int(x.chat_id))
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("❗ لا شي مشغل تم تخطيه!")
    else:
...
async def mute(_, message: Message):
    ...
    await message.reply_text("✅ مكتوم")
    if mute:
        result == 0
    else:
        await message.reply_text("❌ مكتوم بالفعل")
    if not mute:
        result == 1
    else:
        await message.reply_text("❌ لا يوجد مكالمة")
...
async def unmute(_, message: Message):
    ...
    await message.reply_text("✅ تم الغاء الكتم")
    if unmute:
        result == 0
    else:
        await message.reply_text("❌ غير مكتوم")
    if not unmute:
        result == 1
    else:
        await message.reply_text("❌ لا يوجد مكالمة")
